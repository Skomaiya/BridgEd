...
class IsOwnProfile(permissions.BasePermission):
    """User can only access their own profile or own resources (e.g. Resume belongs to student)."""
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.user.role == "employer":
            ...
            return Match.objects.filter(
                student=obj, job__employer__user=request.user, student_interested=True
            ).exists()
