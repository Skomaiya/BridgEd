...
from api.models import User, Student, Employer, Job, Match, Conversation, Message, Notification
...
class TestStartConversation:
    ...
    def test_student_cannot_initiate_conversation(
        self, student_client, accepted_match
    ):
    ...
    def test_employer_cannot_start_conversation_without_match_id(
        self, employer_client
    ):
        response = employer_client.post(
...
class TestListConversations:
    ...
    def test_conversation_list_includes_last_message_preview(
        ...
        assert MESSAGE_BODY[:80] in conv_data['last_message']['body'], (
            f"last_message.body should contain the message text, got: {conv_data['last_message']}"
        )

    def test_conversation_list_empty_when_no_conversations(
        self, employer_client
    ):
        response = employer_client.get(reverse('api:conversation-list'))
        assert response.status_code == status.HTTP_200_OK
...
class TestMessages:
    ...
    def messages_url(self, conversation_id):
        return reverse('api:conversation-messages', kwargs={'conversation_id': conversation_id})

    def test_employer_can_send_message(
        self, employer_client, existing_conversation
    ):
    ...
    def test_sending_message_notifies_recipient(
        ...
        assert Notification.objects.filter(
            ...
            "Sending a message should create a 'new message' notification for the recipient (student)"
        )
    ...
    def test_employer_can_read_messages(
        ...
        assert response.data[0]['body'] == MESSAGE_BODY, (
