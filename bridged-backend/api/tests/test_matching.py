...
from api.models import User, Student, Employer, Job, Match, Resume

@pytest.fixture
def student_user(db):
    user = User.objects.create_user(email="student@test.com", password="Securepass1", role="student")
    Student.objects.create(user=user, display_name="Jane", location="Remote", contract_preferences=[])
    return user
...
def student_with_resume(student_user):
    ...
    Resume.objects.create(
        student=student,
        parsed_data={"technical_skills": ["Python", "Django", "AWS"], "soft_skills": ["Leadership"]},
        status="completed"
    )
    return student
...
class TestMatchGeneration:
    ...
    def test_student_with_resume_gets_matches(self, student_client, student_with_resume, python_job):
    ...
    def test_matches_stored_in_database(self, student_client, student_with_resume, python_job):
    ...
    def test_match_response_has_correct_shape(self, student_client, student_with_resume, python_job):
        ...
        matches = response.data["matches"] if "results" not in response.data else response.data["results"]
        if matches:
            match = matches[0]
    ...
    def test_match_score_not_exposed_to_student(self, student_client, student_with_resume, python_job):
...
class TestMatchInterestDecline:
    ...
    def test_student_can_indicate_interest(self, student_client, existing_match):
        ...
        url = reverse('api:indicate-interest', kwargs={'match_id': existing_match.match_id})
    ...
    def test_employer_notified_when_student_accepts(self, student_client, existing_match, employer_user):
        """When a student accepts, the job's employer receives a student_interested notification."""
        from api.models import Notification
        url = reverse('api:indicate-interest', kwargs={'match_id': existing_match.match_id})
        student_client.post(url)
        assert Notification.objects.filter(user=employer_user, type='student interested').exists()
    ...
    def test_student_can_decline_match(self, student_client, existing_match):
        ...
        url = reverse('api:indicate-decline', kwargs={'match_id': existing_match.match_id})
        response = student_client.post(url)
        assert response.status_code == status.HTTP_200_OK
        existing_match.refresh_from_db()
    ...
    def test_student_cannot_indicate_interest_on_others_match(self, existing_match):
        ...
        other_user = User.objects.create_user(email="anotherstudent@test.com", password="Securepass1", role="student")
...
class TestEmployerMatchesView:
    ...
    def test_student_info_anonymized_before_acceptance(self, employer_client, existing_match):
    ...
    def test_student_info_revealed_after_acceptance(self, student_client, employer_client, existing_match, student_user):
        ...
        data = response.data
        results = data.get("results", data)
        match_data = next((m for m in results if str(m["match_id"]) == str(existing_match.match_id)), None)
        assert match_data is not None
        assert match_data["student"]["anonymized"] is False
        assert match_data["student"]["email"] == student_user.email

    def test_declined_matches_hidden_from_employer(self, student_client, employer_client, existing_match):
...
class TestJobShortlist:
    ...
    def test_employer_can_retrieve_shortlist(self, employer_client, python_job, existing_match):
    ...
    def test_shortlist_respects_max_size(self, employer_client, employer_user, student_with_resume):
        """Shortlist caps to max_shortlist_size when set."""
        emp = employer_user.employer_profile
        job = Job.objects.create(
            employer=emp, title="Capped", description="X",
            required_skills=["Python"], location="Remote", max_shortlist_size=1
        )
        other_user = User.objects.create_user(email="s2@test.com", password="p", role="student")
        other_student = Student.objects.create(user=other_user)
        Match.objects.create(student=student_with_resume, job=job, compatibility_score=90.0)
    ...
    def test_other_employer_cannot_see_shortlist(self, python_job):
        ...
        other_user = User.objects.create_user(email="sneaky@test.com", password="p", role="employer")
