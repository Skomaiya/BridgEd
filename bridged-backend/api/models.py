...
class Student(models.Model):
    ...
    profile_completion_percentage = models.FloatField(default=0.0, validators=[MinValueValidator(0.0), MaxValueValidator(100.0)])
...
class Resume(models.Model):
    ...
    file = models.FileField(upload_to='resumes/%Y/%m/', max_length=500, null=True, blank=True)
    ...
    parsing_accuracy = models.FloatField(null=True, blank=True, validators=[MinValueValidator(0.0), MaxValueValidator(1.0)])
...
class Job(models.Model):
    ...
    contract_type = models.CharField(max_length=50, choices=CONTRACT_TYPE_CHOICES, default='internship')
    ...
    def check_expiry(self):
        ...
        from django.utils import timezone
        if self.is_open and self.application_deadline and self.application_deadline < timezone.now():
            self.is_open = False
...
class Match(models.Model):
    ...
    compatibility_score = models.FloatField(validators=[MinValueValidator(0.0), MaxValueValidator(100.0)])
...
class ContactRequest(models.Model):
    ...
    def __str__(self):
        return f"Contact from {self.name} ({self.email}) - {self.subject or 'No Subject'}"
