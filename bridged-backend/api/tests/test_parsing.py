...
class TestResumeUpload:
    ...
    def test_student_can_upload_pdf_returns_201_with_resume_id(self, student_api):
        ...
        with patch(
            "services.supabase_storage.is_supabase_configured", return_value=False
        ), patch("api.views._run_parsing_in_background"):
            api_response = student_api.post(
                reverse(self.UPLOAD_URL), {"file": make_pdf_file()}, format="multipart"
            )
    ...
    def test_student_can_upload_docx_file(self, student_api):
        ...
        with patch(
            ...
            api_response = student_api.post(
                reverse(self.UPLOAD_URL), {"file": make_docx_file()}, format="multipart"
            )
        assert api_response.status_code == status.HTTP_201_CREATED
    ...
    def test_student_can_upload_txt_file(self, student_api):
        ...
        with patch(
            ...
            api_response = student_api.post(
                reverse(self.UPLOAD_URL), {"file": make_txt_file()}, format="multipart"
            )
        assert api_response.status_code == status.HTTP_201_CREATED

    def test_uploading_second_cv_replaces_first_record(
        self, student_api, student_account
    ):
        ...
        first_resume_record = Resume.objects.create(
            student=student_profile, status="completed"
        )
        with patch(
            "services.supabase_storage.is_supabase_configured", return_value=False
        ), patch("api.views._run_parsing_in_background"):
        ...
        assert not Resume.objects.filter(
            resume_id=first_resume_record.resume_id
        ).exists()
    ...
    def test_upload_with_no_file_field_returns_400(self, student_api):
        ...
        api_response = student_api.post(
            reverse(self.UPLOAD_URL), {}, format="multipart"
        )
    ...
    def test_image_file_type_is_rejected_with_400(self, student_api):
        ...
        image_file = SimpleUploadedFile(
            "photo.jpg", b"pixel data", content_type="image/jpeg"
        )
        api_response = student_api.post(
            reverse(self.UPLOAD_URL), {"file": image_file}, format="multipart"
        )
        assert api_response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestResumeDetail:
    ...
    def test_student_can_poll_own_resume_status(self, student_api, student_account):
        ...
        student_profile = student_account.student_profile
        processing_resume = Resume.objects.create(
            student=student_profile, status="processing"
        )
        api_response = student_api.get(
            reverse(self.DETAIL_URL, kwargs={"resume_id": processing_resume.resume_id})
        )
    ...
    def test_completed_resume_exposes_parsed_data(self, student_api, student_account):
        ...
        completed_resume = Resume.objects.create(
            student=student_profile, parsed_data=MOCK_PARSED_DATA, status="completed"
        )
        api_response = student_api.get(
    ...
    def test_student_can_patch_parsed_data_to_correct_errors(
        self, student_api, student_account
    ):
    ...
    def test_another_student_cannot_view_someone_elses_resume(self, student_account):
        ...
        private_resume = Resume.objects.create(
            student=original_student_profile, status="processing"
        )

        intruder_account = User.objects.create_user(
            email="intruder@test.com", password="Securepass1", role="student"
        )
...
class TestResumeParsingPipeline:
    ...
    def _make_pipeline(
        self, MockExtractor, MockParser, extracted_text="Jane Doe, Python Developer"
    ):
    ...
    def test_run_delegates_to_extractor_then_parser_in_order(self):
        ...
        with patch("services.resume_pipeline.TextExtractor") as MockExtractor, patch(
            "services.resume_pipeline.LLMResumeParser"
        ) as MockParser, patch("services.resume_pipeline.log_device_info"), patch(
    ...
    def test_run_propagates_text_extraction_error(self):
        ...
        with patch("services.resume_pipeline.TextExtractor") as MockExtractor, patch(
            "services.resume_pipeline.LLMResumeParser"
        ), patch("services.resume_pipeline.log_device_info"), patch(
            "os.path.exists", return_value=True
        ):
...
class TestRealResumeExtraction:
    ...
    def test_extraction_result_is_single_string_not_list(
        self, resume_filename, resume_path
    ):
    ...
    def test_extracted_text_contains_at_least_one_alphabetic_word(
        self, resume_filename, resume_path
    ):
        ...
        words_found = [
            word for word in extracted_text.split() if word.isalpha() and len(word) > 1
        ]
...
class TestInvalidFilePaths:
    ...
    def test_extractor_raises_for_nonexistent_file_path(self):
        ...
        from services.text_extractor import TextExtractionError, TextExtractor
