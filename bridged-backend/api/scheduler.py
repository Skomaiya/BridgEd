...
def start():
    ...
    if not enabled:
        logger.info("Matching scheduler disabled via settings. Skipping scheduler startup.")
        return
    ...
    scheduler.add_job(run_scan, CronTrigger(hour=8, minute=0), id="matching_scan_morning", replace_existing=True)
    scheduler.add_job(run_scan, CronTrigger(hour=20, minute=0), id="matching_scan_evening", replace_existing=True)
