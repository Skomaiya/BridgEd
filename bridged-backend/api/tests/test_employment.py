...
from api.models import User, Student, Employer, Job, Match, Notification

@pytest.fixture
def employer_user(db):
    user = User.objects.create_user(email="employer_test@test.com", password="Securepass1", role="employer")
...
@pytest.fixture
def student_user(db):
    user = User.objects.create_user(email="student_test@test.com", password="Securepass1", role="student")
...
class TestEmploymentDismissal:
    def test_employ_candidate(self, employer_client, employer_user, student_user, open_job):
        student = student_user.student_profile
        match = Match.objects.create(job=open_job, student=student, compatibility_score=85.0)
        ...
        assert Notification.objects.filter(user=student_user, type='interest confirmed').exists()

    def test_job_closes_when_filled(self, employer_client, employer_user, student_user, open_job):
        student = student_user.student_profile
    ...
    def test_dismiss_candidate(self, employer_client, employer_user, student_user, open_job):
        student = student_user.student_profile
        match = Match.objects.create(job=open_job, student=student, compatibility_score=85.0)
    ...
    def test_matching_engine_filters_dismissed(self, student_user, open_job):
        ...
        Match.objects.create(job=open_job, student=student, compatibility_score=85.0, status='dismissed')
