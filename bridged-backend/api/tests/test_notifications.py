...
def other_student(db):
    user = User.objects.create_user(email="other@test.com", password="Securepass1", role="student")
    Student.objects.create(user=user)
    return user
...
class TestNotificationListing:
    ...
    def test_student_sees_own_notifications(self, student_client, student_user):
        ...
        Notification.objects.create(user=student_user, type='new match', message="Match 1")
        Notification.objects.create(user=student_user, type='job posted', message="Job alert")
        response = student_client.get(reverse(self.URL))
        assert response.status_code == status.HTTP_200_OK
        results = response.data.get("results", response.data)
        assert len(results) == 2

    def test_student_does_not_see_others_notifications(self, student_client, other_student):
        """Students cannot see notifications belonging to other users."""
        Notification.objects.create(user=other_student, type='new match', message="Hidden")
        response = student_client.get(reverse(self.URL))
        results = response.data.get("results", response.data)
    ...
    def test_all_notification_types_are_retrievable(self, student_client, student_user):
        ...
        types = ['new match', 'job posted', 'job published', 'cv parsed', 'subscription expiring']
        for t in types:
...
class TestMarkNotificationRead:
    ...
    def test_student_can_mark_notification_as_read(self, student_client, student_user):
        ...
        n = Notification.objects.create(user=student_user, type='new match', message="!!", is_read=False)
        response = student_client.post(reverse(self.URL, kwargs={'notification_id': n.notification_id}))
        assert response.status_code == status.HTTP_200_OK
        n.refresh_from_db()
        assert n.is_read is True

    def test_marking_already_read_notification_is_idempotent(self, student_client, student_user):
        """Marking a notification that is already read does not cause an error."""
        n = Notification.objects.create(user=student_user, type='new match', message="Done", is_read=True)
        response = student_client.post(reverse(self.URL, kwargs={'notification_id': n.notification_id}))
        assert response.status_code == status.HTTP_200_OK

    def test_student_cannot_mark_others_notification_as_read(self, student_client, other_student):
        """Students cannot mark notifications belonging to another user as read."""
        n = Notification.objects.create(user=other_student, type='new match', message="Private")
        response = student_client.post(reverse(self.URL, kwargs={'notification_id': n.notification_id}))
    ...
    def test_nonexistent_notification_returns_404(self, student_client):
        ...
        import uuid
        fake_id = uuid.uuid4()
        response = student_client.post(reverse(self.URL, kwargs={'notification_id': fake_id}))
    ...
    def test_unauthenticated_cannot_mark_notification_read(self, db, student_user):
        ...
        n = Notification.objects.create(user=student_user, type='new match', message="!!!")
        response = APIClient().post(reverse(self.URL, kwargs={'notification_id': n.notification_id}))
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
...
class TestNotificationDeduplication:
    ...
    def test_match_notification_has_1hr_cooldown(self, student_user):
        ...
        Notification.objects.create(user=student_user, type='new match', message="Just matched!")
    ...
    def test_notification_older_than_1hr_is_not_recent(self, student_user):
        ...
        old_notif = Notification.objects.create(user=student_user, type='new match', message="Old")
