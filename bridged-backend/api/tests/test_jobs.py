...
from api.models import User, Student, Employer, Job, Notification
...
def second_employer(db):
    user = User.objects.create_user(email="other@test.com", password="Securepass1", role="employer")
...
class TestJobCreation:
    ...
    def test_employer_assigned_automatically(self, employer_client, employer_user):
        ...
        employer_client.post(reverse(self.URL), {
            "title": "Auto Assigned",
    ...
    def test_employer_notified_when_job_goes_live(self, employer_client, employer_user):
        ...
        assert Notification.objects.filter(user=employer_user, type='job published').exists()
...
class TestJobRetrieval:
    ...
    def test_closed_jobs_hidden_from_authenticated_list(self, student_client, employer_user):
        """Closed jobs do not appear in the public job list."""
        emp = employer_user.employer_profile
        Job.objects.create(employer=emp, title="Closed Listing", description="X", required_skills=[], location="R", is_open=False)
        response = student_client.get(reverse(self.LIST_URL))
        results = response.data.get("results", response.data)
    ...
    def test_employer_my_jobs_includes_closed_listings(self, employer_client, employer_user):
        """The my_jobs action returns all employer jobs, including closed ones."""
        emp = employer_user.employer_profile
        Job.objects.create(employer=emp, title="Old Role", description="X", required_skills=[], location="R", is_open=False)
    ...
    def test_job_search_by_title(self, student_client, open_job):
        ...
        response = student_client.get(reverse(self.LIST_URL), {"search": "Software Engineer"})
        results = response.data.get("results", response.data)
        assert any(j["title"] == "Software Engineer" for j in results)
