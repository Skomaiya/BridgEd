...
from .models import (
    Student, Employer, Resume, Job, Match, Subscription, 
    Notification, ContactRequest, Conversation, Message, UserReport
)
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['user_id', 'email', 'role', 'is_active', 'profile_image_url', 'profile_image_storage_path', 'notification_preferences', 'created_at']
...
class ContactRequestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'request_id', 'user', 'name', 'email', 
            'subject', 'message', 'is_resolved', 'created_at'
        ]
...
class StudentSerializer(serializers.ModelSerializer):
    ...
    profile_completion_percentage = serializers.FloatField(required=False, allow_null=True)
    profile_image_url = serializers.URLField(source='user.profile_image_url', read_only=True)
    profile_image_storage_path = serializers.CharField(source='user.profile_image_storage_path', read_only=True)

    class Meta:
        model = Student
        fields = [
            'student_id', 'user', 'university', 'course',
            'expected_graduation_year', 'location',
            'linkedin_url', 'additional_links',
            'is_premium_active', 'profile_completion_percentage',
            'profile_image_url', 'profile_image_storage_path',
            'display_name', 'subscription_plan',
            'contract_preferences',
            'created_at', 'updated_at',
        ]
        read_only_fields = [
            'student_id', 'created_at', 'updated_at', 'is_premium_active',
            'profile_image_storage_path', 'subscription_plan'
        ]
    ...
    def validate_linkedin_url(self, value):
        ...
        if not url.startswith(('http://', 'https://')):
            raise serializers.ValidationError("Please provide the full URL starting with https://")
        return url
    ...
    def validate_additional_links(self, value):
        ...
        return [
            {"link_type": str(item.get("link_type", "")).strip(), "url": str(item.get("url", "")).strip()}
            for item in value
        ]
...
class EmployerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['employer_id', 'user', 'company_name', 'industry',
                  ...
                  'profile_image_url', 'profile_image_storage_path',
                  'created_at', 'updated_at']
        read_only_fields = ['employer_id', 'created_at', 'updated_at', 'is_verified', 'profile_image_storage_path']
...
class ResumeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Resume
        fields = [
            'resume_id', 'student', 'file', 'file_url', 'parsed_data',
            'parsing_accuracy', 'uploaded_at', 'status', 'parsing_error',
        ]
        read_only_fields = [
            'resume_id', 'file_url', 'parsed_data', 'parsing_accuracy',
            'uploaded_at', 'status', 'parsing_error',
        ]
...
class JobSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Job
        fields = [
            'job_id', 'employer', 'employer_id', 'title', 'description',
            'required_skills', 'nice_to_have_skills', 'location',
            'contract_type', 'job_length',
            'is_open', 'published_at', 'application_deadline', 
            'max_shortlist_size', 'recruitment_slots', 'hired_count',
            'created_at', 'updated_at',
        ]
    ...
    def to_representation(self, instance):
        ...
        if instance.application_deadline and instance.application_deadline < timezone.now():
            ret['is_open'] = False
        return ret
...
class JobListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'contract_type', 'job_length', 'required_skills', 'is_open', 
...
class MatchSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['match_id', 'student', 'job', 'compatibility_score',
                  'student_interested', 'status', 'matched_at']
...
class AdminEmployerSerializer(EmployerSerializer):
    ...
    class Meta(EmployerSerializer.Meta):
        read_only_fields = [f for f in EmployerSerializer.Meta.read_only_fields if f != 'is_verified']
...
class AdminStudentSerializer(StudentSerializer):
    ...
    class Meta(StudentSerializer.Meta):
        read_only_fields = [
            f for f in StudentSerializer.Meta.read_only_fields 
...
class AdminUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'profile_image_url', 'profile_image_storage_path',
            'student_profile', 'employer_profile'
        ]
        read_only_fields = ['user_id', 'created_at']
...
class MessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Message
        fields = ['message_id', 'conversation', 'sender', 'sender_email', 'sender_role',
                  'body', 'is_read', 'sent_at', 'edited_at', 'is_edited']
        read_only_fields = ['message_id', 'conversation', 'sender', 'sender_email',
                            'sender_role', 'is_read', 'sent_at', 'edited_at', 'is_edited']
...
class ConversationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Conversation
        fields = ['conversation_id', 'match_id', 'other_party_name',
                  'other_party_user_id', 'last_message', 'unread_count', 
                  'created_at']
...
class UserReportSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserReport
        fields = [
            'report_id', 'reporter', 'reporter_email', 
