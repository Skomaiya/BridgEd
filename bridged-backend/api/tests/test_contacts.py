...
from api.models import User, Student, Employer, ContactRequest, Notification
...
def employer_user(db):
    user = User.objects.create_user(email="emp@test.com", password="Securepass1", role="employer")
...
class TestContactSubmit:
    ...
    def test_submission_notifies_admins(self, db):
        ...
        admin = User.objects.create_superuser(email="admin@here.com", password="Admin1234")
        APIClient().post(reverse(self.URL), {
            "name": "Someone",
...
class TestAdminContactManagement:
    ...
    def test_resolving_already_resolved_is_idempotent(self, admin_client):
        ...
        client, _ = admin_client
        req = ContactRequest.objects.create(name="D", email="d@d.com", message="Already done", is_resolved=True)
        response = client.post(reverse(self.RESOLVE_URL, kwargs={'pk': req.request_id}))
        assert response.status_code == status.HTTP_200_OK
