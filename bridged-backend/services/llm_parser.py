...
def _normalize_parsed(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for item in safe_list(_get_key(raw, "education") or []):
        if isinstance(item, dict):
            education.append({
                ...
                "field": safe_str(_get_key(item, "field", "Field", "field of study")),
                "institution": safe_str(_get_key(item, "institution", "Institution", "school")),
                "location": safe_str(_get_key(item, "location", "Location")),
                "start_date": safe_str(_get_key(item, "start_date", "Start Date", "start")),
    ...
    for item in safe_list(_get_key(raw, "certifications") or []):
        if isinstance(item, dict):
            cname = safe_str(_get_key(item, "name", "title", "Title")) or (str(item).strip() if item else None)
            cissuer = safe_str(_get_key(item, "issuer", "provider", "Provider"))
            certifications.append({"name": cname or str(item).strip(), "issuer": cissuer})
        else:
    ...
    for item in safe_list(_get_key(raw, "experience") or []):
        if isinstance(item, dict):
            resp = _get_key(item, "responsibilities", "Responsibilities", "description", "Description", "duties")
            if isinstance(resp, list):
                ...
                responsibilities = []
            experience.append({
                "title": safe_str(_get_key(item, "title", "Title", "job title", "position", "Position")),
                "company": safe_str(_get_key(item, "company", "Company", "employer", "Employer")),
                "location": safe_str(_get_key(item, "location", "Location")),
                "start_date": safe_str(_get_key(item, "start_date", "Start Date", "start")),
                "end_date": safe_str(_get_key(item, "end_date", "End Date", "end")),
    ...
    soft_list = safe_list(soft_val) if not isinstance(soft_val, str) else [s.strip() for s in re.split(r"[,;\n|]", str(soft_val)) if s.strip()]
    soft_skills = [x.lower() for s in soft_list if (x := skill_to_string(s))]

    confidence = safe_float(_get_key(raw, "confidence"), 0.0)
    if (confidence == 0.0 or confidence < 0.3) and (name or technical_skills or education or experience):
        confidence = min(0.95, 0.1 * (
            (0.15 if name else 0) + (0.15 if _get_key(raw, "email") else 0) + (0.1 if _get_key(raw, "phone") else 0) +
            (0.2 * min(1.0, len(technical_skills) / 10.0)) +
            (0.2 if education else 0) + (0.2 if experience else 0)
        ))
    ...
    return {
        ...
        "languages": [str(s).strip() for s in safe_list(_get_key(raw, "languages") or []) if str(s).strip()],
        "education": education,
...
class LLMResumeParser:
    ...
    def parse(self, text: str) -> Dict[str, Any]:
        ...
        messages = [
            {"role": "system", "content": "Output only valid JSON. No markdown, no comments."},
        ...
        try:
            ...
            if "response_format" in str(e).lower() or "json_object" in str(e).lower() or "unexpected keyword" in str(e).lower():
                response = client.chat.completions.create(
                    model=self.model,
                    messages=messages,
                    temperature=0.0,
