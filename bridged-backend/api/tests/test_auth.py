...
class TestRegistration:
    ...
    def test_employer_registers_and_creates_profile(self, client):
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert Employer.objects.filter(user__email="employer@test.com", company_name="Acme Ltd").exists()

    def test_registration_notifies_admins(self, client):
        """Admins receive a user_registered notification when a new user signs up."""
        admin = User.objects.create_superuser(email="admin@test.com", password="adminPass1")
    ...
    def test_duplicate_email_rejected(self, client):
        ...
        User.objects.create_user(email="dupe@test.com", password="Securepass1", role="student")
        response = client.post(reverse(self.URL), {
            "email": "dupe@test.com",
            "password": "Securepass1",
            "password_confirm": "Securepass1",
...
class TestLogin:
    ...
    def test_login_success_returns_tokens(self, client):
        ...
        User.objects.create_user(email="login@test.com", password="Securepass1", role="student")
        response = client.post(reverse(self.URL), {
    ...
    def test_login_returns_user_role(self, client):
        ...
        User.objects.create_user(email="emp@test.com", password="Securepass1", role="employer")
        response = client.post(reverse(self.URL), {
            "email": "emp@test.com",
            "password": "Securepass1"
        })
    ...
    def test_wrong_password_returns_401(self, client):
        ...
        User.objects.create_user(email="user@test.com", password="Securepass1", role="student")
        response = client.post(reverse(self.URL), {
            "email": "user@test.com",
            "password": "WrongPass1"
        })
    ...
    def test_suspended_account_returns_error(self, client):
        ...
        user = User.objects.create_user(email="banned@test.com", password="Securepass1", role="student")
...
class TestLogout:
    ...
    def test_valid_token_blacklisted_on_logout(self, db):
        ...
        user = User.objects.create_user(email="out@test.com", password="Securepass1", role="student")
    ...
    def test_invalid_token_returns_400(self, db):
        ...
        user = User.objects.create_user(email="out2@test.com", password="Securepass1", role="student")
        client = APIClient()
        client.force_authenticate(user=user)
...
class TestDeleteAccount:
    ...
    def test_user_can_delete_own_account(self, db):
        ...
        user = User.objects.create_user(email="del@test.com", password="Securepass1", role="student")
