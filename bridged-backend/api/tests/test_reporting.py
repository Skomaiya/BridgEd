...
from api.models import User, Student, Employer, UserReport

@pytest.fixture
def student_user(db):
    user = User.objects.create_user(email='reporter@test.com', password='testpass123', role='student')
    Student.objects.create(user=user, display_name='Reporter Student')
    return user

@pytest.fixture
def reported_user(db):
    user = User.objects.create_user(email='reported@test.com', password='testpass123', role='employer')
    Employer.objects.create(user=user, company_name='Reported Corp')
    return user

@pytest.fixture
def admin_user(db):
    return User.objects.create_superuser(email='admin@test.com', password='adminpass123', role='admin')
