...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        from api.models import Student, Job, Match, Resume, Notification
        ...
        students = (
            ...
            .filter(resume__status=Resume.STATUS_COMPLETED)
        )
        
        now = timezone.now()
        expired_count = Job.objects.filter(is_open=True, application_deadline__lt=now).update(is_open=False)
        if expired_count > 0:
        ...
        for student in students:
            try:
                ...
                for job in open_jobs:
                    match_result = engine.calculate_match(student_skills, job.required_skills, job.nice_to_have_skills)
