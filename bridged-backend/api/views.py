...
from .models import Student, Employer, Resume, Job, Match, Subscription, Notification, ContactRequest, Conversation, Message, UserReport
from .serializers import (
    UserSerializer, UserRegistrationSerializer, StudentSerializer,
    EmployerSerializer, ResumeSerializer, ResumeUpdateSerializer,
    JobSerializer, JobListSerializer, MatchSerializer, MatchListSerializer,
    SubscriptionSerializer, NotificationSerializer, ContactRequestSerializer,
    LoginSerializer, LogoutSerializer, AuthResponseSerializer, FileUploadSerializer,
    AdminUserSerializer, AdminEmployerSerializer, AdminStudentSerializer,
    ConversationSerializer, MessageSerializer, UserReportSerializer,
)
...
class ContactRequestView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {'message': 'Your enquiry has been received and is being processed.', 'data': serializer.data}, 
                status=status.HTTP_201_CREATED
            )
...
def _handle_photo_upload(user, request, role_folder):
    ...
    if not file:
        ...
        return Response({'error': f'No file provided (Content-Type: {ct})'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    if not is_supabase_configured():
        return Response({'error': 'Profile photos are not configured (Supabase).'}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
...
def _run_parsing_in_background(
    ...
    from services.resume_pipeline import ResumeParsingPipeline, PipelineExecutionError
    from services.supabase_storage import upload_resume_file, is_supabase_configured
    ...
    try:
        ...
        try:
            resume_obj = Resume.objects.select_related('student__user').get(resume_id=resume_id)
...
def _recompute_matches_for_job(job):
    ...
    from .models import Student, Match, Notification
...
class JobViewSet(viewsets.ModelViewSet):
    ...
    def my_jobs(self, request):
        ...
        queryset = Job.objects.filter(employer__user=request.user).order_by('-created_at')
    ...
    @action(detail=True, methods=['get'], permission_classes=[IsAuthenticated, IsEmployer], url_path='shortlist')
    def shortlist(self, request, job_id=None):
        ...
        matches = Match.objects.filter(match_id__in=selected_ids).select_related('student', 'student__user').order_by('-compatibility_score')
...
class MatchView(APIView):
    ...
    def post(self, request):
        try:
                ...
                if score >= STORE_THRESHOLD:
                    ...
                    if score >= SHOW_THRESHOLD:
                        ...
                        if deadline is not None:
                            deadline_date = deadline.date() if hasattr(deadline, 'date') and callable(deadline.date) else deadline
                        ...
                        if cap is not None:
                            ...
                            ).exclude(match_id=match.match_id).count()
                        ...
                        match_payload['company_name'] = match_payload.get('company') or job.employer.company_name
                        match_payload['employer_bio'] = getattr(job.employer, 'bio', '') or ''
                        match_payload['employer_industry'] = getattr(job.employer, 'industry', '') or ''
                        match_payload['employer_company_size'] = getattr(job.employer, 'company_size', '') or ''
                        match_payload['employer_website'] = getattr(job.employer, 'website', '') or ''
                        match_payload['employer_location'] = getattr(job.employer, 'location', '') or ''
                        match_payload['is_open'] = job.is_open
                        match_payload['application_deadline'] = job.application_deadline.isoformat() if job.application_deadline else None
            ...
            matches_data.sort(key=lambda x: len(x.get('matched_skills', [])), reverse=True)
            ...
            if new_match_count > 0 and any_new_match:
                ...
                if not recent_notif:
                    if new_match_count == 1:
                        ...
                        extra = f" and {new_match_count - 3} more" if new_match_count > 3 else ""
                        msg = (
    ...
    def post(self, request, match_id):
        try:
            match = Match.objects.select_related(
                'student', 'student__user', 'job', 'job__employer', 'job__employer__user'
            ).get(
                match_id=match_id,
        ...
        for m in results_to_serialize:
            out.append({
                ...
                'student': {
                    ...
                    'display_name': m.student.display_name if m.student_interested else None,
        ...
        try:
            ...
            if resume.file_storage_path:
                try:
                    ...
                    signed_url = create_signed_resume_url(resume.file_storage_path, expires_in=3600)
                    if signed_url:
...
class PaystackVerifyView(APIView):
    ...
    def post(self, request):
        ...
        secret_key = getattr(django_settings, 'PAYSTACK_SECRET_KEY', '')
        if not secret_key:
            return Response({'error': 'Payment is not configured on this server.'}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
        ...
        try:
            ...
            return Response({'error': f'Verification failed: {exc}'}, status=status.HTTP_502_BAD_GATEWAY)
        ...
        plan_name = (
            PLAN_CODE_LOOKUP.get(plan_code)
        ...
        if request.user.role == 'student':
            try:
                ...
                logger.info('Updated student %s profile: plan=%s, is_premium=%s', 
                             request.user.email, student.subscription_plan, student.is_premium_active)
            except Exception as e:
                logger.warning('Could not update student profile after payment: %s', e)

        plan_display = SUBSCRIPTION_PLANS.get(plan_name, {}).get('name', plan_name or 'Unknown')
        Notification.objects.create(
            user=request.user,
            type='new match',
...
class AdminUserViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = [filters.SearchFilter]
    search_fields = ['email', 'student_profile__display_name', 'employer_profile__company_name']
    queryset = User.objects.all().select_related('student_profile', 'employer_profile').order_by('-created_at')
    ...
    def perform_destroy(self, instance):
        ...
        if hasattr(instance, 'student_profile') and hasattr(instance.student_profile, 'resume'):
    ...
    def verify_employer(self, request, user_id=None):
        ...
        if employer.is_verified:
            admins = User.objects.filter(role='admin', notification_preferences__verified_employer=True)
            notifications = [
                Notification(
                    user=admin,
                    type='profile update',
    ...
    def toggle_active(self, request, user_id=None):
        ...
        admin_users = User.objects.filter(role='admin', notification_preferences__user_suspended=True)
        notifications = [
            Notification(
                user=admin,
                type='user suspended',
...
class ConversationViewSet(viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == 'employer':
            return Conversation.objects.filter(employer__user=user).select_related('match', 'employer', 'student__user').prefetch_related('messages__sender')
        else:
            return Conversation.objects.filter(student__user=user).select_related('match', 'employer', 'student__user').prefetch_related('messages__sender')
    ...
    def create(self, request):
        ...
        try:
            match = Match.objects.select_related('student__user', 'job__employer__user').get(
                match_id=match_id,
...
class EmployMatchView(APIView):
    ...
    def post(self, request, pk):
        if request.user.role != 'employer':
            return Response({'error': 'Only employers can hire.'}, status=status.HTTP_403_FORBIDDEN)
        ...
        if match.status == 'employed':
            return Response({'error': 'Candidate already hired.'}, status=status.HTTP_400_BAD_REQUEST)
        
        job = match.job
        if job.hired_count >= job.recruitment_slots:
             return Response({'error': 'No recruitment slots left.'}, status=status.HTTP_400_BAD_REQUEST)
...
class DismissMatchView(APIView):
    ...
    def post(self, request, pk):
        if request.user.role != 'employer':
            return Response({'error': 'Only employers can dismiss candidates.'}, status=status.HTTP_403_FORBIDDEN)
        
        try:
            match = Match.objects.get(pk=pk, job__employer=request.user.employer_profile)
        except Match.DoesNotExist:
            return Response({'error': 'Match not found.'}, status=status.HTTP_404_NOT_FOUND)
...
class MessageListView(APIView):
    ...
    def patch(self, request, conversation_id):
        ...
        if time_diff.total_seconds() > 300:
            return Response({'error': 'Message can only be edited within 5 minutes of sending.'}, status=status.HTTP_400_BAD_REQUEST)
